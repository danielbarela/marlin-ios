...
class DataSourceMap2: MapMixin {
    var uuid: UUID = UUID()
    var cancellable = Set<AnyCancellable>()
    var minZoom = 2

    var repository: TileRepository2?
    var mapFeatureRepository: MapFeatureRepository?
    ...
    init(repository: TileRepository2? = nil, mapFeatureRepository: MapFeatureRepository? = nil) {
        self.repository = repository
        self.mapFeatureRepository = mapFeatureRepository
    }
    ...
    func updateMixin(mapView: MKMapView, mapState: MapState) {
        ...
        if lastChange == nil
            ...
            if let repository = repository {
                let newOverlay = DataSourceTileOverlay2(tileRepository: repository, key: dataSourceKey)
                newOverlay.tileSize = CGSize(width: 512, height: 512)
                newOverlay.minimumZ = self.minZoom
